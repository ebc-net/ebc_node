...
//                    setSrvAddr(clientInfo,aport);
                ...
                //把对方节点加入服务器的K桶
                Sp<Node> clinode = std::make_shared<Node> (srcId);
...
{
    for(auto it = sockNodePair.begin();it!=sockNodePair.end();it++)
    {
        if(it->second->getId()==dstId)
        {
            memset(buf, 0, BUFSIZE);
            UDT::sendmsg(it->first,buf,ret);
            break;
        }
    }
}
